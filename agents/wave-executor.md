...
**Concurrency cap.** When `EXECENV=on`, each env is a real container + database. Run at most
`ENV_CAP` task worktrees at a time: split the wave's parallel groups into batches of `ENV_CAP`
(`0` = no cap) and process batches sequentially — provision batch → spawn batch → cherry-pick →
teardown batch → next batch. Never provision the whole wave up front.
