...
| Rule | Trigger | Severity |
|---|---|---|
| No task deps anywhere | manifest has no `task_deps` AND no task body has `depends_on:` | **MED** — `NO_TASK_DEPS_WAVE_BARRIER`, non-blocking (legacy shape / pre-v0.22.0 PLAN) |
| Partial adoption | ≥1 task has `depends_on:`, ≥1 does not | HIGH — the graph is half-specified; the executor cannot trust it |
| Dependency cycle | `depends_on` graph has a cycle | **BLOCKER** — the executor refuses to start a cyclic set |
| Dangling dep | `depends_on` names a T-id that does not exist in the PLAN | **BLOCKER** |
| `task_deps` disagrees with a task body | same T-id, different dep list | HIGH (executor reads the manifest) |
| Manifest without `critical_path` | `task_deps` present, `critical_path` absent | MED |
| File-collision encoded as a dependency | two tasks share a file AND one lists the other in `depends_on` with no data/contract reason in `action:` | MED (the executor already serializes file collisions — the dep only narrows the graph) |
| Serial-tail wave | terminal REFACTOR/SECURITY wave whose `files_touched` spans ≥2 other waves' files, with no cross-cutting justification in any of its tasks' `action:` | MED (see `feature-planner` → `<task_dependencies>`: task-scoped refactor/security belongs to the task) |
| Critical path ≈ the whole phase | `critical_path_length` ≥ 80% of `task_count` | MED (the plan is a chain — fan-out cannot help it) |

Report the resulting shape even when everything passes: `depth` (longest chain), `width` (count of
`depends_on: []` tasks — how wide the phase opens) and the theoretical max concurrency. A phase with
width 1 will run serially no matter the cap, and that is worth saying out loud.

Only cycles and dangling ids FAIL. Everything else is advisory — the gate informs the user that the
plan will underuse the machine; it does not refuse to run a correct-but-narrow plan.
...
- `PASS` — zero ORPHAN tasks, zero UNCOVERED goals, zero BLOCKER stack-gate violations, zero advanced-threat-gate violations (every declared dangerous surface has its required test/check), zero complexity-label BLOCKERs, AND zero dependency-graph BLOCKERs (no cycle, no dangling T-id)
- `FAIL` — any BLOCKER finding (orphan, uncovered, stack-gate BLOCKER, advanced-threat-gate BLOCKER — a dangerous surface introduced without its matching A1/A2/A3/A11/A12/A13 test or `check_*` static gate — a task missing/misdeclaring `complexity:`, or a cyclic/dangling `depends_on`)
...
- ALWAYS run every audit step (forward + backward + stack gates + complexity labels + dependency graph) even after first BLOCKER — surface all gaps
- NEVER FAIL a plan merely for missing `depends_on` — that is MED (`NO_TASK_DEPS_WAVE_BARRIER`). Only a CYCLE or a dangling T-id is a BLOCKER: those make the plan unschedulable, not just slow
- NEVER add or infer a dependency for the planner — report the shape (depth / width / max concurrency) and let the user decide whether a narrow plan is acceptable
...
scheduler_shape:                      # v0.22.0 — what the readiness scheduler will be able to do
  ...
  max_theoretical_concurrency: {N}
  cycles: []                          # BLOCKER when non-empty
  dangling_deps: []                   # BLOCKER when non-empty
...
|--------|-------|---------|
| task_deps present | yes/no | no ⇒ executor runs wave-barrier mode; re-plan to unlock fan-out |
| depth (longest chain) | {N} | the wall-clock floor — no cap shortens it |
| width (`depends_on: []`) | {N} | how many tasks can start at t=0 |
| max theoretical concurrency | {N} | compare against the machine cap min(8, cores/2) |
| cycles / dangling | — | **BLOCKER** when present |

Critical path: T01 → T05 → T12 ({N} of {N} tasks).
