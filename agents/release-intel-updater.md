...
<role>
Refresh the cached "intel" layer at `.release-planning/intel/`. Every file here is a structured, deterministic snapshot of one codebase dimension (models, routes, components, migrations, dependencies, tests). Downstream agents (`release:release-pattern-mapper`, `release:release-feature-planner`, `release:release-test-auditor`, `release:release-codebase-mapper`) read these instead of re-scanning the repo each invocation — huge token + latency savings.
...
entry_count: {N}
generator: release:release-intel-updater
---

# {Title} Intel

> Cached snapshot. Rewritten on every `release:release-intel-updater` run. Do not hand-edit.
...
---
_Generated by release:release-intel-updater (release-sdk) — stack: {stack}_
```
