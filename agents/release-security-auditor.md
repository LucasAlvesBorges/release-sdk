...
- Mitigation grep: `BLACKLIST_AFTER_ROTATION = True`, `ROTATE_REFRESH_TOKENS = True`, sane lifetimes, logout blacklists refresh
- Test grep: `test_*jwt_*`, expired → 401; logout → next refresh → 401

**Cat 6: Input Validation / Injection**
- Threat: SQL injection, command injection, path traversal, stored XSS
- Mitigation grep: NO `.raw(...)` with f-string, NO `.extra(where=[...])` with user input, serializer fields have `validators=[...]` or type-narrowed
- Test grep: `test_*injection*`, payload `'; DROP TABLE` → 400 or sanitized
...
- Mitigation grep: SessionAuthentication endpoints have CSRF protection, NO `@csrf_exempt` on session-auth endpoints (JWT-only can opt out with documented reason)
- Test grep: `test_*csrf*`, request without CSRF → 403

**Cat 9: Cookie / Token Security**
- Threat: token theft via XSS, MITM, cross-origin
- Mitigation grep: JWT in `httpOnly` + `Secure` + `SameSite` cookie (NOT localStorage), `SECURE_SSL_REDIRECT = True`, `SESSION_COOKIE_SECURE = True`, CORS allowlist explicit (NOT `CORS_ALLOW_ALL_ORIGINS=True`)
- Test grep: `test_*cookie_security*`, Set-Cookie has HttpOnly+Secure+SameSite

### BLOCKER triggers (auto-OPEN)
- `@csrf_exempt` on session-auth endpoint without documented reason
- `fields = '__all__'`
- `.raw(...)` or `.extra(where=[...])` with user input
