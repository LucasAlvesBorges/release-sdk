...
Appends a `## Researcher Findings` section to the existing AI-SPEC.md. Consumed by django-feature-planner (backend pipeline) and react-feature-planner (frontend pipeline) when /release:plan --fullstack runs.
</role>
...
- DO NOT write PLAN.md — that's django-feature-planner + react-feature-planner during /release:plan --fullstack.
