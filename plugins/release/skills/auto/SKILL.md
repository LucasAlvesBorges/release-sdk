...
- `gsd-planner` → `release-feature-planner`
- `gsd-executor` → `release-tdd-executor`
- `gsd-code-reviewer` → `release-code-reviewer`
- `gsd-verifier` → `release-phase-verifier`
- `gsd-phase-researcher` → `release-feature-researcher`
...
- `gsd-roadmapper` → `release:new-milestone` (skill; no direct agent spawn)
...
Deriving from the session model guarantees a spawn NEVER requests a tier the user lacks (workers are always exactly one rung below the orchestrator). bash cannot read the session model (no env var — only `$CODEX_REASONING_EFFORT`), so YOU set it from self-knowledge; there is nothing to prompt the user for.
...
Also instruct every worker spawn to "operate at maximum rigor / max effort" in its prompt (subagent effort is not yet a spawnable param; `$CODEX_REASONING_EFFORT` on this session is already `max`). If the lib is unreachable, fall back to the frontmatter defaults (worker→sonnet, checker→opus) and say so.
