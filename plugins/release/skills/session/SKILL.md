...
name: session
description: >
  Worktree-native parallel sessions (v0.16.0). Spin up N isolated Codex sessions off one
...
git worktree on a `session/<label>` branch cut from a **base branch**. Launch one Codex session
...
Open a Codex session there and work normally:
  cd ../release-worktrees/sessions/financeiro && codex
