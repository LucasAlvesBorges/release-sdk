...
// Only when the plugin runtime exports RELEASE_PLUGIN_ROOT and no PLUGIN_DATA (i.e. not Codex): make
...
function ensureTokenWorker() {
  ...
  if (process.env.RELEASE_TOKEN_AUTOSTART === '0' || !process.env.RELEASE_PLUGIN_ROOT || process.env.PLUGIN_DATA) return;
  const worker = path.join(process.env.RELEASE_PLUGIN_ROOT, 'bin', 'release-token-worker.js');
...
function gcHint() {
  if (event !== 'SessionStart' || !process.env.RELEASE_PLUGIN_ROOT || process.env.RELEASE_GC_HINT === '0') return '';
  const lib = path.join(process.env.RELEASE_PLUGIN_ROOT, 'bin', 'release-gc-lib.sh');
